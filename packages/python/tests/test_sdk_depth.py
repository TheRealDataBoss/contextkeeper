...
from contextkeeper.exceptions import ContextKeeperError, HandoffNotFoundError
from contextkeeper.models import AgentType, TaskStatus


@pytest.fixture
def client(tmp_path: Path) -> ContextKeeperClient:
    c = ContextKeeperClient(project_dir=tmp_path)
    c.init(name="SDK Test")
    return c


@pytest.fixture
def client_with_handoff(client: ContextKeeperClient) -> ContextKeeperClient:
    client.sync(
        notes="seed",
...
class TestSessionManagement:
    def test_open_session(self, client: ContextKeeperClient):
        session = client.open_session(agent="claude")
        assert session.agent == AgentType.claude
        assert session.closed_at is None

    def test_open_multiple_sessions(self, client: ContextKeeperClient):
    ...
    def test_close_session_by_id(self, client: ContextKeeperClient):
        session = client.open_session()
        closed = client.close_session(session.id)
        assert closed.closed_at is not None
        assert closed.id == session.id

    def test_close_current_session(self, client: ContextKeeperClient):
        session = client.open_session()
        closed = client.close_session()
        assert closed.closed_at is not None
        assert closed.id == session.id

    def test_close_no_open_session_raises(self, client: ContextKeeperClient):
        s = client.open_session()
        client.close_session(s.id)
        with pytest.raises(ContextKeeperError, match="No open sessions"):
            client.close_session()

    def test_get_session(self, client: ContextKeeperClient):
        session = client.open_session(agent="gpt")
        fetched = client.get_session(session.id)
        assert fetched.id == session.id
        assert fetched.agent == AgentType.gpt

    def test_list_sessions_newest_first(self, client: ContextKeeperClient):
        s1 = client.open_session()
        s2 = client.open_session()
        sessions = client.list_sessions()
    ...
    def test_list_sessions_empty(self, client: ContextKeeperClient):
        sessions = client.list_sessions()
        assert sessions == []
...
class TestHandoffManagement:
    def test_get_handoff(self, client_with_handoff: ContextKeeperClient):
    ...
    def test_list_handoffs_newest_first(self, client_with_handoff: ContextKeeperClient):
    ...
    def test_get_handoff_wrong_version_raises(self, client_with_handoff: ContextKeeperClient):
        sessions = client_with_handoff.list_sessions()
        session_id = sessions[0].id
        with pytest.raises(Exception):
            client_with_handoff.get_handoff(session_id, 999)

    def test_list_handoffs_no_handoffs(self, client: ContextKeeperClient):
        s = client.open_session()
...
class TestTaskManagement:
    def test_add_task(self, client_with_handoff: ContextKeeperClient):
    ...
    def test_add_task_upserts_existing(self, client_with_handoff: ContextKeeperClient):
        handoff = client_with_handoff.add_task(
    ...
    def test_task_creates_new_version(self, client_with_handoff: ContextKeeperClient):
...
class TestDecisionManagement:
    def test_add_decision(self, client_with_handoff: ContextKeeperClient):
    ...
    def test_add_decision_no_handoff_raises(self, client: ContextKeeperClient):
        with pytest.raises(HandoffNotFoundError):
            client.add_decision(decision_id="DEC-0001", summary="Nope")

    def test_add_decision_with_supersedes(self, client_with_handoff: ContextKeeperClient):
        handoff = client_with_handoff.add_decision(
    ...
    def test_decision_creates_new_version(self, client_with_handoff: ContextKeeperClient):
...
class TestExport:
    def test_export_returns_briefing(self, client_with_handoff: ContextKeeperClient):
        briefing = client_with_handoff.export_briefing()
        assert "SDK Test" in briefing or "sdk-test" in briefing

    def test_export_to_file(self, client_with_handoff: ContextKeeperClient, tmp_path: Path):
    ...
    def test_export_no_handoff(self, client: ContextKeeperClient):
        briefing = client.export_briefing()
        assert "no handoffs" in briefing.lower() or "sync" in briefing.lower()

    def test_export_to_file_no_handoff(self, client: ContextKeeperClient, tmp_path: Path):
