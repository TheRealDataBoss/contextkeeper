"""Tests for ContextKeeperClient — full lifecycle with real FileBackend."""
...
from contextkeeper.exceptions import ProjectNotInitializedError
from contextkeeper.models import AgentType, TaskStatus


@pytest.fixture
def client(tmp_path: Path) -> ContextKeeperClient:
    return ContextKeeperClient(project_dir=tmp_path)


@pytest.fixture
def initialized_client(client: ContextKeeperClient) -> ContextKeeperClient:
...
class TestInit:
    def test_creates_project(self, client: ContextKeeperClient):
        config = client.init(name="My Project")
        assert config.project_id == "my-project"
        assert config.name == "My Project"
        assert config.coordination == "sequential"

    def test_custom_coordination(self, client: ContextKeeperClient):
        config = client.init(name="Locked", coordination="lock")
        assert config.coordination == "lock"

    def test_slugifies_name(self, client: ContextKeeperClient):
...
class TestSync:
    def test_creates_handoff(self, initialized_client: ContextKeeperClient):
    ...
    def test_increments_version(self, initialized_client: ContextKeeperClient):
    ...
    def test_with_tasks(self, initialized_client: ContextKeeperClient):
    ...
    def test_with_decisions(self, initialized_client: ContextKeeperClient):
        handoff = initialized_client.sync(
            decisions=[
    ...
    def test_with_agent(self, initialized_client: ContextKeeperClient):
    ...
    def test_with_questions_and_next_steps(self, initialized_client: ContextKeeperClient):
        handoff = initialized_client.sync(
    ...
    def test_with_next_steps_only(self, initialized_client: ContextKeeperClient):
        handoff = initialized_client.sync(
    ...
    def test_with_open_questions_only(self, initialized_client: ContextKeeperClient):
        handoff = initialized_client.sync(
    ...
    def test_with_tasks_list(self, initialized_client: ContextKeeperClient):
        handoff = initialized_client.sync(
            tasks=[
...
class TestBootstrap:
    def test_no_handoffs_message(self, initialized_client: ContextKeeperClient):
        output = initialized_client.bootstrap()
        assert "no handoffs" in output.lower()

    def test_renders_briefing(self, initialized_client: ContextKeeperClient):
...
class TestStatus:
    def test_empty_project(self, initialized_client: ContextKeeperClient):
    ...
    def test_after_sync(self, initialized_client: ContextKeeperClient):
        initialized_client.sync(
            tasks=[
...
class TestDoctor:
    def test_healthy_project(self, initialized_client: ContextKeeperClient):
        result = initialized_client.doctor()
        assert result["healthy"] is True
        assert all(c["status"] != "fail" for c in result["checks"])

    def test_uninitialized_project(self, client: ContextKeeperClient):
...
class TestDiff:
    def test_diff_between_versions(self, initialized_client: ContextKeeperClient):
...
class TestFullLifecycle:
    def test_init_sync_bootstrap_status_doctor(self, tmp_path: Path):
        ...
        client = ContextKeeperClient(project_dir=tmp_path)
