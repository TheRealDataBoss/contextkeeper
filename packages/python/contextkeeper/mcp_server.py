"""contextkeeper MCP server — expose tools via Model Context Protocol."""
...
def _client() -> ContextKeeperClient:
    return ContextKeeperClient()
...
def contextkeeper_init(
    ...
    """Initialize a contextkeeper project in the current directory.
...
@mcp.tool()
def contextkeeper_bootstrap() -> str:
...
@mcp.tool()
def contextkeeper_update_task(task_id: str, status: str) -> str:
...
@mcp.tool()
def contextkeeper_add_decision(
    decision_id: str,
    summary: str,
    rationale: str = "",
    made_by: str = "human",
...
@mcp.tool()
def contextkeeper_list_sessions() -> str:
...
@mcp.tool()
def contextkeeper_diff(from_version: int, to_version: int) -> str:
