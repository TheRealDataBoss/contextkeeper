"""contextkeeper doctor -- environment and configuration health check."""
...
def run_doctor() -> None:
    ...
    console.print("\n  [cyan]contextkeeper doctor[/cyan]\n")
    ...
    # 5. ~/.contextkeeperrc exists and has token
    rc_path = Path.home() / ".contextkeeperrc"
    if rc_path.exists():
        try:
            rc_data = json.loads(rc_path.read_text(encoding="utf-8"))
            token = rc_data.get("npm_token") or rc_data.get("token")
            if token:
                console.print(f"  {PASS} GitHub token found in ~/.contextkeeperrc")
                passed += 1
            else:
                console.print(f"  {FAIL} ~/.contextkeeperrc exists but no token -- run: contextkeeper sync to configure")
                failed += 1
        except Exception:
            console.print(f"  {FAIL} ~/.contextkeeperrc exists but is not valid JSON")
            failed += 1
    else:
        console.print(f"  {FAIL} ~/.contextkeeperrc not found -- run: contextkeeper sync to configure")
        failed += 1
    ...
    if sv_found:
        ...
        console.print(f"  {FAIL} STATE_VECTOR.json not found -- run: contextkeeper init")
        failed += 1
