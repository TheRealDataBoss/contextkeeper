...
The protocol schema uses semantic versioning (`workbench-v<major>.<minor>`):
...
1. `protocol/workbench.schema.json`
...
Workbench AI is designed for thin wrappers around an identical protocol. To add a new delivery mechanism (e.g., a Rust CLI, a Homebrew formula, a Docker image):
