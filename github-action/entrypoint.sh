...
# workbench-ai GitHub Action entrypoint
# Syncs STATE_VECTOR.json and HANDOFF.md to the workbench bridge repo
...
if command -v jq >/dev/null 2>&1; then
    ...
    if ! echo "${SCHEMA_VER}" | grep -qE '^workbench-v[0-9]+\.[0-9]+$'; then
        echo "::error::Invalid schema_version: ${SCHEMA_VER}. Expected pattern: workbench-v<major>.<minor>"
        exit 1
    fi
...
BRIDGE_URL="https://x-access-token:${GH_TOKEN}@github.com/${WORKBENCH_REPO}.git"

git clone --depth 1 "${BRIDGE_URL}" "${BRIDGE_DIR}" 2>/dev/null
if [ $? -ne 0 ]; then
    echo "::error::Failed to clone bridge repo: ${WORKBENCH_REPO}"
    exit 1
fi

echo "Cloned ${WORKBENCH_REPO}"
echo "::endgroup::"
...
git config user.name "workbench-ai[bot]"
git config user.email "workbench-ai[bot]@users.noreply.github.com"

git add .

COMMIT_MSG="chore(workbench): sync ${PROJECT_NAME} -- ${TIMESTAMP}"
...
echo "::notice::Workbench sync complete for ${PROJECT_NAME} (${COMMIT_SHA})"
